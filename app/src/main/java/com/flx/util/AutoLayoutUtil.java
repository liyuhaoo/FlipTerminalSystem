 ...
 * @Description:屏幕适配工具类
 *
 *                      使用说明：http://blog.csdn.net/zhengjingle/article/details/51742839
 * @date 2017-2-21 下午4:41:51
 * @author liyuhao
 */
public class AutoLayoutUtil {
	...
	public static void setSize(Activity act, boolean hasStatusBar, int designWidth, int designHeight) {
		...
		AutoLayoutUtil.displayWidth = width;
		AutoLayoutUtil.displayHeight = height;

		AutoLayoutUtil.designWidth = designWidth;
		AutoLayoutUtil.designHeight = designHeight;

		double displayDiagonal = Math.sqrt(Math.pow(AutoLayoutUtil.displayWidth, 2) + Math.pow(AutoLayoutUtil.displayHeight, 2));
		double designDiagonal = Math.sqrt(Math.pow(AutoLayoutUtil.designWidth, 2) + Math.pow(AutoLayoutUtil.designHeight, 2));
		AutoLayoutUtil.textPixelsRate = displayDiagonal / designDiagonal;
	}
	...
	public static void auto(View view) {
		...
		AutoLayoutUtil.autoTextSize(view);
		AutoLayoutUtil.autoSize(view);
		AutoLayoutUtil.autoPadding(view);
		AutoLayoutUtil.autoMargin(view);
